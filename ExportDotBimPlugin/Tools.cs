...
namespace export_DOTBIM
{
    public static class Tools
    {
        ...
        public static Color GetBimColorFromDrawingColor(System.Drawing.Color drawingColor)
        {
        ...
        public static dotbim.Mesh CreateBimMeshFromRhinoMesh(Mesh mesh, int meshId)
        {
        ...
        public static File CreateFile(List<Element> elements, List<dotbim.Mesh> meshes, Dictionary<string, string> info)
        {
            File file = new File
            {
                SchemaVersion = "1.0.0",
        ...
        public static Vector ConvertInsertPlaneToVector(Plane plane)
        {
        ...
        public static Rotation ConvertInsertPlaneToRotation(Plane plane)
        {
        ...
        public static List<Mesh> ConvertBimMeshesAndElementsIntoRhinoMeshes(List<dotbim.Mesh> bimMeshes, List<Element> bimElements)
        {
            ...
            List<Rhino.DocObjects.Material> materials = new List<Rhino.DocObjects.Material>();
            ...
            foreach (var bimElement in bimElements)
            {
                ...
                else
                {
                    ...
                    for (var i = 0; i < meshReferenced.Faces.Count; i++)
                    {
                        ...
                        meshReferenced.MergeAllCoplanarFaces(0.001);
        ...
        public static bool chooseColor = false;

        public static void SetFacesColor(Element bimElement, Mesh rhinoMesh)
        {
            if (rhinoMesh == null)
            {
                throw new ArgumentException("Invalid Rhino mesh provided");
            }
            ...
            if (rhinoMesh.VertexColors.Count > 0)
            {
                ...
                {
                    colors.Add(rhinoMesh.VertexColors[f.A]);
                }
                for (int i = 0; i < rhinoMesh.Faces.Count; i++)
                {
                    faceColors.Add(colors[i].R);
                    faceColors.Add(colors[i].G);
                    faceColors.Add(colors[i].B);
                    faceColors.Add(colors[i].A);
                }
                bimElement.FaceColors = faceColors;
            }
            else
                chooseColor = false;
        }
        ...
        public static void SetFacesColor(Element bimElement, Mesh rhinoMesh)
        {
            ...
            else
            {
                for (int i = 0; i < rhinoMesh.Faces.Count; i++)
                {
                    var center = rhinoMesh.Faces.GetFaceCenter(i);
                    MeshPoint meshPoint = rhinoMesh.ClosestMeshPoint(center, 0);
                    System.Drawing.Color color = rhinoMesh.ColorAt(meshPoint);
                    colors.Add(color);
                }
            }
            ...
            for (int i = 0; i < rhinoMesh.Faces.Count; i++)
            {
                //colors.Add(rhinoMesh.VertexColors[rhinoMesh.Faces[i].A]);
                faceColors.Add(colors[i].R);
                faceColors.Add(colors[i].G);
                faceColors.Add(colors[i].B);
                faceColors.Add(colors[i].A);
            }
            // Set the face colors of the element
            bimElement.FaceColors = faceColors;
        }
        /*/
    }
